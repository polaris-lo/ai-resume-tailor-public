...
| **首次配置** | 在「⚙️ 设置」页面填写并保存，之后自动加载 | 需要运行 `python setup.py` | 无需额外配置 |
...
**首次使用**：点击「⚙️ 设置」标签页，填写姓名、上传默认简历、配置 API Key，点击「保存设置」。配置保存在 `~/.resume-tailor/config.json`，之后每次打开自动加载，无需重复填写。

**日常使用**：

1. 在「📝 简历定制」标签页粘贴 JD（简历和 API Key 已自动加载）
2. 点击「分析并生成修改建议」
3. 审阅建议，可输入意见让 LLM 重新生成
4. 确认后下载定制版 Word 简历
